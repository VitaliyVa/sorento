...
$('.add_btn').on('click', function () {
  var wrap = $(this).parents('.order_card');
...
function basket_delete() {
  ...
  var item_id = $(this).attr('data-quantity_item_id'); //   fetch(`/api/cart_item/${item_id}`, {
  //     method: 'DELETE',
  //   })
  //     .then(data => {
  //       return data.json();
...
$('.radio_block').on('click', function () {
  var wrap = $(this).parents('.radio__wrap');
...
$('.select_city').select2({
  dropdownAutoWidth: true,
  width: 'resolve' // matcher: matchCustom
